...
densiTree <- function(x, type = "cladogram", alpha = 1 / length(x),
                      ...
                      font = 3, tip.color = 1, adj = 0, srt = 0,flipval=1000000,
                      underscore = FALSE, label.offset = 0, scale.bar = TRUE,usetrees=NA,
  ...
  if (direction == "rightwards") {
    ...
    plot.default(0, type = "n", xlim = c(0, 1+sw), ylim = c(0, nTip + 1),
  ...
  if (direction == "leftwards") {
    ...
    plot.default(0, type = "n", xlim = c(-log10(2.0*maxBT), log10(2.0*maxBT)), ylim = c(-5, nTip + 1),
                 xlab = "", ylab = "", axes = FALSE, ...)
    
    seq(1,2.0*maxBT)
    
    # logseq(0, log(2.0*maxBT), n = 5)
    print(maxBT)
    
    sequence = log10(c(1,11, 121,1021))
    
    breaksforaxis = sort(unique(c(-sequence,sequence)))
    labels = 2020-(10^abs(breaksforaxis)-1)
    ...
    axis(side = 1, at = breaksforaxis,labels = as.character(labels))
  }
  if (direction == "downwards") {
  ...
  for (treeindex in seq_along(x)) {
    if (is.element(treeindex, usetrees)){
      ...
      if (type == "cladogram") {
        ...
        if (treeindex<=flipval)
          xx=-1*xx
        
        cladogram.plot(tmp$edge, xx, yy, edge.color =
