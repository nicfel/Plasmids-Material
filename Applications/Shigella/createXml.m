...
for dataset_nr = 1 : 2
    ...
    elseif dataset_nr==2
        ...
        use_seqs = sort(randsample(use_seqs, min(length(use_seqs),250)));
    ...
    for r = 0:2
        ...
        if dataset_nr==1
            g = fopen(['xmls/Sonnei_rep' num2str(r) '.xml'], 'w');
        else
            g = fopen(['xmls/Flexneri_rep' num2str(r) '.xml'], 'w');
        end
...
%                  fprintf(g, '\t\t<run id="mcmc" spec="beast.core.MCMC" chainLength="10000000" storeEvery="1000000">\n');            
            ...
            elseif contains(line, 'insert_parameters')
                ...
                for s = 1 : length(segments)
                    ...
                    fprintf(g, '\t\t\t\t<parameter id="mutationRate.s:%s" name="stateNode">0.0005</parameter>\n',segments{s});
...
%                         fprintf(g, '\t\t\t\t\t\t</branchRateModel>\n');
    ...
    for r = 0 : 2
        ...
        for seg = 1 :length(segments)
            ...
            if dataset_nr==1
                g = fopen(sprintf('xmls/Sonnei_%s_rep%d.xml', segments{seg}, r), 'w');
            else
                g = fopen(sprintf('xmls/Flexneri_%s_rep%d.xml', segments{seg}, r), 'w');
            end                
